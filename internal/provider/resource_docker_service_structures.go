...
func supportsServiceContainerInit(client *client.Client) bool {
	return client != nil && client.ClientVersion() >= "1.37"
}
...
func flattenContainerSpec(in *swarm.ContainerSpec, client *client.Client) []interface{} {
	...
	if supportsServiceContainerInit(client) {
		if in.Init != nil {
			m["init"] = *in.Init
		}
	}
		...
		for _, rawContainerSpec := range v.([]interface{}) {
			...
			if supportsServiceContainerInit(client) {
				if value, ok := rawContainerSpec["init"]; ok {
					initEnabled := value.(bool)
					containerSpec.Init = &initEnabled
				}
			}
...
func extraHostsSetToServiceExtraHosts(extraHosts *schema.Set) []string {
	...
	for _, hostInt := range extraHosts.List() {
		...
		// see https://github.com/gocommunity-io/terraform-provider-dockerd/issues/202#issuecomment-847715879
